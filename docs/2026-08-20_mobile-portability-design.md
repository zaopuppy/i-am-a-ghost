...
- 日期：2026-08-20
- 更新：2026-08-21
- 状态：Gate A/H3 已通过两机可玩房间会话；H4 的高频 bridge、稳定性与性能门槛仍待验证
...
### Gate A/H3 真机结果（2026-08-21）

H3 最小可玩闭环已在 Pura X 与 nova 16 Pro 的同一可互访局域网上通过：

- Pura X 点击“创建房间”后进入本地房主大厅；nova 16 Pro 的附近列表自动出现六位码、人数和“加入”按钮。点击后，两端显示同一房间码和同一份两人名单，只有房主端显示可用的“开始游戏”。
- 房主开始后，两端同时退出大厅并进入对局；一次实测中 Pura X 得到小孩 viewer、nova 16 Pro 得到鬼 viewer，两端倒计时一致并持续推进权威 tick。二维码、附近房间和 bridge 调试浮层在开局后收起，不再遮挡游戏画面。
- 房主 ArkWeb Web Worker 运行共享 `MatchEngine`，ArkTS 通过持久 TCP 定向转发 `room-state`、20 Hz viewer frame、事件和 30 Hz 输入。自动化测试还验证远端 peer 加入、两人名单、房主开始以及远端持续收到 `match-frame`。
- 两台真机分别拖动左下摇杆后，CDP 只读诊断显示双方 viewer 的权威位置都发生变化：小孩从 `(-13.5, -7.2)` 移到约 `(-12.02, -9.54)`，鬼从 `(0, 0)` 移到约 `(0.68, -2.59)`；手势结束后 movement 回到零。
- 小孩端长按右下手电按钮期间以 100 ms 间隔采样 30 次，其中 13 次观测到 `actionHeld=true`，松开后恢复 `false`。控件处理 `pointerup`、`pointercancel`、失焦和页面隐藏清零；鬼端只显示摇杆。
- 触控 HUD 使用左右安全区和至少 96 CSS px 的主要触控面，电量、声音和对局 HUD 已避让；两台真机横屏截图未见裁切或系统白带。一次截图两端均显示 60 FPS，但这只是瞬时读数，不代替 H4 性能统计。
- `npm run prototype:harmony:build` 通过且 ArkTS 无编译 warning；`devecocli check lint` 无缺陷；98 项规则/服务器测试和 4 项聚焦 Playwright 测试通过。两台真机均安装同一签名 HAP，未发现 crash。临时 ArkWeb DevTools 端口转发已在验证后删除。

因此 H3 通过，但 Gate A 整体仍未通过。尚未完成：可互访网络上的真实二维码加入、房主退后台时的对局中断页复测、`WebMessagePort` / `ArrayBuffer` 30/20 Hz 替换当前 25 ms JavaScript-proxy 轮询、15 分钟持续运行、P95 帧时间/温度/内存，以及桌面协议客户端到手机主机的跨宿主一致性。

### Gate A/H3 合入前加固（2026-08-21）

在把原型分支合入主干前，H3 对权威房间边界做了以下收口；这些改动只提高原型的正确性和可维护性，不改变 Gate A 尚未整体通过的结论：

- Harmony 房间消息类型归入共享 `src/net/protocol.ts`，并在 Worker 入口逐字段校验合法 JSON；同一 peer 的重复 `requestId` 返回缓存响应，不会重复执行动作。
- 原生 TCP listener 限制最多 8 条并发连接（为短连接房间探测预留余量），权威房间仍限制为房主加 4 个远端玩家；Worker 对输入消息限频，并继续执行 64 KiB 帧长与输入范围检查。
- 桌面服务器与 Harmony Worker 共用角色轮换、输入新鲜度和手电有效性规则；过期移动/动作输入不会继续驱动角色，电量耗尽时不会继续投影手电。
- 页面进入后台时，Harmony 客户端立即终止权威 Worker、关闭原生房间、移除二维码/扫描入口并回到大厅。浏览器自动化覆盖资源清理；真机对局中断提示仍保留为单独复测项。
- 自动化新增非法消息、请求去重、输入过期、手电电量和后台清理回归；最终合入门禁为 102 项规则/服务器测试、28 项 Playwright 测试全部通过，`devecocli check lint` 为 0 缺陷，签名 HAP 构建成功。

仍明确留给 H4：桌面协议客户端加入手机主机、`WebMessagePort` / `ArrayBuffer` 高频链路、可互访网络上的真实二维码加入、真机后台中断页复测，以及 15 分钟稳定性和 P95 帧时间/温度/内存测量。
