...
M0–M6 实现与自动 QA 已完成：当前版本可供 2–5 人在本机或局域网创建邀请码房间、自主选择鬼或小孩、完成五分钟对局并重新准备。服务端拥有阵营选择、移动、阻挡、抓取、照射、电池、计时与胜负；小孩客户端在鬼未被照中时拿不到鬼坐标。
...
- 大厅中的每位玩家都要明确选择“鬼”或“小孩”；每局只有一个公开的鬼席位，以服务器最先收到的选择为准。
- 至少两名在线玩家、全员完成选择且恰好一人为鬼时，房主才能开始首局。结算后全员重新选阵营并准备下一局；系统不会随机补位或自动轮换鬼。
- 鬼：`WASD` 或方向键移动；持续接触小孩约 `0.3` 秒后完成抓捕，无需额外按键。
- 小孩（网页版）：`WASD` 或方向键移动；在游戏画面内按住鼠标左键照明并朝鼠标瞄准，松开关灯。
- 小孩（鸿蒙版）：左摇杆移动，右摇杆按住照明、拖动转向；重新按下时先沿当前朝向照明，松开关灯。
- 小孩松开照明后，移动时约 100ms 自动转向前进方向，静止时保留朝向；按住但电量耗尽时仍保持瞄准模式。
- 小孩前进、横移、后退速度为基速的 100%、95%、90%，根据移动方向与上半身朝向连续变化。横移时下半身最多偏转 45°，上半身与手电保持瞄准方向。
- 鬼被手电照中后进入灼烧状态，期间减速且无法抓取，只能移动逃离。
- 鬼抓到小孩三次获胜；鬼生命归零或五分钟倒计时归零则小孩获胜。
- 小孩低电时地图生成一块全角色可见的电池；头灯随鬼接近由慢闪、快闪变为常亮。
- 对局加载开始后角色固定。玩家掉线时角色停在原地并继续参与碰撞、抓捕、伤害和胜负；对局不会中断，玩家在本局结束前重连可恢复原角色控制。

不需要账号或数据库；房间、昵称、阵营选择与重连凭据都只在当前进程/浏览器会话中存在。鸿蒙权威房主设备关闭时房间仍会结束，本版本不迁移主机。
